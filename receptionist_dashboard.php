<?php
// receptionist_dashboard.php — admin-only. Lets the receptionist view
// doctors / patients / appointments / prescriptions / contact-form
// messages, add doctors (with hashed passwords), and delete doctors.
// All actions require role=admin. All queries are parameterized.
// Password columns are NEVER echoed to the UI.
...
// Add a doctor. Password is hashed with password_hash() before storing.
...
if (isset($_POST['docsub'])) {
    ...
    mysqli_stmt_bind_param($stmt, "sssss", $doctor, $hashed, $demail, $spec, $docFees);
    ...
    if ($ok) {
        ...
        echo "<script>alert('Unable to add doctor.');</script>";
    }
}
          ...
          <div class="tab-content" id="nav-tabContent" style="width: 950px;">
                  ...
                  <div class="col-sm-4">
                    <div class="panel panel-white no-radius text-center">
                      <div class="panel-body">
                        ...
                        <p class="links cl-effect-1"><a href="#list-doc" onclick="clickDiv('#list-doc-list')">View Doctors</a></p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="col-sm-4" style="left: -3%">
                    <div class="panel panel-white no-radius text-center">
                      <div class="panel-body">
                        ...
                        <p class="cl-effect-1"><a href="#list-pat" onclick="clickDiv('#list-pat-list')">View Patients</a></p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="col-sm-4">
                    <div class="panel panel-white no-radius text-center">
                      <div class="panel-body">
                        ...
                        <p class="cl-effect-1"><a href="#list-app" onclick="clickDiv('#list-app-list')">View Appointments</a></p>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="col-sm-4" style="left: 13%;margin-top: 5%;">
                    <div class="panel panel-white no-radius text-center">
                      <div class="panel-body">
                        ...
                        <p class="cl-effect-1"><a href="#list-pres" onclick="clickDiv('#list-pres-list')">View Prescriptions</a></p>
                      </div>
                    </div>
                  </div>
            ...
            <div class="tab-pane fade" id="list-doc" role="tabpanel" aria-labelledby="list-home-list">
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <?php
                    // NOTE: we no longer SELECT the password column. Passwords
                    // are hashed and must never be displayed, regardless of role.
                    $result = mysqli_query($con, "SELECT username, spec, email, docFees FROM doctb");
                    while ($row = mysqli_fetch_assoc($result)) {
                        $u_h = htmlspecialchars($row['username'] ?? '', ENT_QUOTES, 'UTF-8');
                        $s_h = htmlspecialchars($row['spec']     ?? '', ENT_QUOTES, 'UTF-8');
                        $e_h = htmlspecialchars($row['email']    ?? '', ENT_QUOTES, 'UTF-8');
                        $f_h = htmlspecialchars((string)($row['docFees'] ?? ''), ENT_QUOTES, 'UTF-8');
                        echo "<tr>
                                <td>{$u_h}</td>
                                <td>{$s_h}</td>
                                <td>{$e_h}</td>
            ...
            <div class="tab-pane fade" id="list-pat" role="tabpanel" aria-labelledby="list-pat-list">
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <?php
                    // NOTE: password column removed from SELECT and display.
                    $result = mysqli_query(
                        $con,
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        $fn_h  = htmlspecialchars($row['fname']   ?? '', ENT_QUOTES, 'UTF-8');
                        $ln_h  = htmlspecialchars($row['lname']   ?? '', ENT_QUOTES, 'UTF-8');
                        $g_h   = htmlspecialchars($row['gender']  ?? '', ENT_QUOTES, 'UTF-8');
                        $e_h   = htmlspecialchars($row['email']   ?? '', ENT_QUOTES, 'UTF-8');
                        $c_h   = htmlspecialchars($row['contact'] ?? '', ENT_QUOTES, 'UTF-8');
                        echo "<tr>
                                <td>{$pid_h}</td>
                                <td>{$fn_h}</td>
                                <td>{$ln_h}</td>
                ...
                <table class="table table-hover">
                  ...
                  <tbody>
                    <?php
                      $result = mysqli_query(
                          $con,
                          "SELECT doctor, pid, ID, fname, lname, appdate, apptime,
                                  disease, allergy, prescription FROM prestb"
                      );
                      while ($row = mysqli_fetch_assoc($result)) {
                          $d_h    = htmlspecialchars($row['doctor']       ?? '', ENT_QUOTES, 'UTF-8');
                          $pid_h  = htmlspecialchars((string)($row['pid'] ?? ''), ENT_QUOTES, 'UTF-8');
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <?php
                    $result = mysqli_query(
                        $con,
                        "SELECT ID, pid, fname, lname, gender, email, contact,
                                doctor, docFees, appdate, apptime, userStatus, doctorStatus
                         FROM appointmenttb"
                    );
                    while ($row = mysqli_fetch_assoc($result)) {
                        $status = '';
                        ...
                        $ad_h    = htmlspecialchars($row['appdate'] ?? '', ENT_QUOTES, 'UTF-8');
                        $at_h    = htmlspecialchars($row['apptime'] ?? '', ENT_QUOTES, 'UTF-8');
                        $stat_h  = htmlspecialchars($status, ENT_QUOTES, 'UTF-8');
...
>>>>>>> master
            ...
            <div class="tab-pane fade" id="list-settings" role="tabpanel" aria-labelledby="list-settings-list">
              <form class="form-group" method="post" action="receptionist_dashboard.php">
                <div class="row">
                  ...
                  <div class="col-md-8" id='cpass'><input type="password" class="form-control" onkeyup='check();' name="cdpassword" id="cdpassword" required>&nbsp;&nbsp;<span id='message'></span></div><br><br>
            ...
            <div class="tab-pane fade" id="list-mes" role="tabpanel" aria-labelledby="list-mes-list">
              ...
              <table class="table table-hover">
                ...
                <tbody>
                  <?php
                    $result = mysqli_query($con, "SELECT name, email, contact, message FROM contact");
                    while ($row = mysqli_fetch_assoc($result)) {
                        $n_h = htmlspecialchars($row['name']    ?? '', ENT_QUOTES, 'UTF-8');
                        $e_h = htmlspecialchars($row['email']   ?? '', ENT_QUOTES, 'UTF-8');
                        $c_h = htmlspecialchars($row['contact'] ?? '', ENT_QUOTES, 'UTF-8');
                        $m_h = htmlspecialchars($row['message'] ?? '', ENT_QUOTES, 'UTF-8');
                        echo "<tr>
                                <td>{$n_h}</td>
                                <td>{$e_h}</td>
                                <td>{$c_h}</td>
